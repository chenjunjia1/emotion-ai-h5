...
export function rowToUser(row: Record<string, unknown>): User {
  return {
    ...
    inviteRewardTotal:
      row.invite_reward_total != null ? Number(row.invite_reward_total) : undefined,
    inviteBlindBoxCount:
      row.invite_blind_box_count != null
        ? Number(row.invite_blind_box_count)
        : undefined,
    growthXp: row.growth_xp != null ? Number(row.growth_xp) : undefined,
