...
function RankBadge({ rank }: { rank: number }) {
  ...
  return (
    <span
      className={cn(
        "absolute left-2 top-2 z-10 flex h-5 min-w-[20px] items-center justify-center rounded-full px-1 text-[10px] font-black text-white shadow-md",
        colors
      )}
    >
      {rank}
...
export function HomeHotTopicsTop3() {
  const { tr } = useAppUi();
  /** 首屏固定 fallback，与 SSR 一致；缓存/API 在 useEffect 里更新 */
  const [picks, setPicks] = useState<HomeCuratedPick[]>(fallbackHomeTop3Picks);
  const [playEnterAnim, setPlayEnterAnim] = useState(false);
  const pickIdsRef = useRef(top3PickIds(picks));

  const pexelsItems = useMemo(
    () =>
      picks
        .filter((p) => !xhsCoverDisplayUrl(p.coverImageUrl))
        .map((p) => ({
          id: p.id,
          title: p.title,
  ...
  useEffect(() => {
    ...
    const fromInspirationPool = resolveHomeTop3FromClientPool();
    if (fromInspirationPool) {
      applyPicks(fromInspirationPool);
    }
    ...
    if (cached && cached.length >= 3) {
      ...
      void loadHomeTop3(fetchHomeTop3FromApi).then((next) => {
        if (cancelled) return;
        const resolved =
          next && next.length >= 3 ? next : fromInspirationPool ?? cached;
        applyPicks(resolved);
      });
      return () => {
        cancelled = true;
    ...
    void loadHomeTop3(fetchHomeTop3FromApi).then((next) => {
      const resolved =
        next && next.length >= 3
          ? next
          : fromInspirationPool ?? fallbackHomeTop3Picks();
      applyPicks(resolved);
    });
  ...
  return (
    <section
      className={cn(
        "content-auto overflow-hidden rounded-[20px] border border-[#FFE8F0]/80 bg-white/95 p-3.5 shadow-[0_8px_28px_rgba(255,120,150,0.1)] backdrop-blur-sm",
        playEnterAnim && "home-section-enter"
      )}
    >
      <div className="mb-3 flex items-end justify-between gap-2 px-0.5">
        <div>
          <div className="flex items-center gap-1.5">
            <span className="flex h-6 w-6 items-center justify-center rounded-xl bg-gradient-to-br from-[#FF4F8B] to-[#FF9A4D] text-white">
      ...
      <div className="grid grid-cols-3 gap-2">
        {picks.map((pick, i) => (
          <Link
            ...
            prefetch={i === 0}
            className={cn(
              "group relative block overflow-hidden rounded-[16px] ring-1 ring-[#FFE8F0]/80 shadow-[0_4px_16px_rgba(255,100,140,0.1)] active:scale-[0.98]",
          ...
          >
            <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#F3F4F6]">
              <RankBadge rank={i + 1} />
              {i === 0 ? (
                <span className="absolute right-2 top-2 z-10 rounded-full bg-[#FF4F8B] px-1.5 py-0.5 text-[8px] font-black text-white shadow-md">
                  {tr("homeTop1HotBadge")}
                </span>
              ) : null}
              <Top3Cover
              ...
              <div className="absolute inset-x-0 bottom-0 p-2 pt-6">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1">
                  <span className="rounded-full bg-white/20 px-1.5 py-0.5 text-[8px] font-bold text-white/95 backdrop-blur-sm">
                    {pick.platform}
                  </span>
